...
The object created was then used  to construct a survival curve using the survfit function. 

**NOTE: Check to see if this is the correct survival curve function/graph to use - by variable?**
...
autoplot(aareg(Surv(data$time, data$stroke) ~ data$age + data$htn + data$diab3cat + data$idealhealthsmk + data$bmi3cat + data$totchol3cat))
...
Broken down, the K-M survival probability of having not having stroke at time, $t_j$, $S(t_j)$ can be calculated using: 
...
**Still need to add in test for collinearity, AIC and goodness of fit**

```{r, echo = FALSE, warning = FALSE, message = FALSE}

##Colinearity/Multi-collinearity?? 
##VIF
## USING ___ AS A CUTOFF, 

##LOF

##AIC - do we need to add this? This is when we're comparing models.
...
Using the cox.zph() function from the {survival} packate, the assumption can be checked.
STILL WORKING ON CODE

```{r, echo = FALSE, warning = FALSE, message = FALSE}


#assump_fit <- coxph(Surv(data$time, data$stroke) ~ age + as.factor(htn) + as.factor(diab3cat) + as.factor(idealhealthsmk) + as.factor(bmi3cat)+ as.factor(totchol3cat), data=data)
